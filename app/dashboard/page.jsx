...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-6 pb-20 font-sans">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4 bg-white p-4 rounded-xl shadow-sm">
          <div className="flex items-center gap-4 w-full md:w-auto justify-center md:justify-start">
            <img src="/logo.png" alt="SDC" className="h-14 w-auto object-contain" onError={(e)=>e.target.style.display='none'} />
          ...
          <button onClick={() => {supabase.auth.signOut(); router.push('/')}} className="text-red-600 px-3 py-1 text-sm border border-red-100 rounded hover:bg-red-50 w-full md:w-auto">SAIR</button>
        </div>

        {/* SELEÇÃO DE ALUNO RESPONSIVA */}
            ...
            {alunos.length > 0 ? (
              <div className="flex items-center gap-3 justify-center md:justify-start">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={abrirEdicao} className="bg-blue-100 text-blue-600 p-2 rounded hover:bg-blue-200" title="Editar">✏️</button>
                </div>
              </div>
            ) : <p className="text-gray-500">Nenhum aluno.</p>}
          </div>
          <button onClick={() => setModalCadastro(true)} className="bg-green-600 text-white px-4 py-2 rounded shadow font-bold text-sm w-full md:w-auto">+ Adicionar Aluno</button>
        </div>

        {alunos.length > 0 && (
          <div className="grid md:grid-cols-2 gap-6">
            <div className="bg-white p-4 md:p-6 rounded-xl shadow-sm border h-fit">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-green-50 p-4 md:p-6 rounded-xl shadow-sm border border-green-100">
      ...
      {/* MODAIS RESPONSIVOS (LARGURA CORRIGIDA) */}
      {modalEditar && (<div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center p-4 z-50"><div className="bg-white rounded-xl shadow-2xl w-[95%] md:w-full max-w-lg p-6 relative"><button onClick={()=>setModalEditar(false)} className="absolute top-4 right-4 text-gray-400">X</button><h2 className="text-xl font-bold text-gray-800 mb-4">Editar Aluno</h2><form onSubmit={salvarEdicao} className="space-y-3"><input className="w-full border p-2 rounded text-black" value={formEdicao.nome} onChange={e=>setFormEdicao({...formEdicao, nome:e.target.value})} /><div className="flex gap-2"><input className="w-1/2 border p-2 rounded text-black" value={formEdicao.posicao} onChange={e=>setFormEdicao({...formEdicao, posicao:e.target.value})} placeholder="Posição" /><input className="w-1/2 border p-2 rounded text-black" type="date" value={formEdicao.data_nascimento} onChange={e=>setFormEdicao({...formEdicao, data_nascimento:e.target.value})} /></div><textarea className="w-full border p-2 rounded text-black" value={formEdicao.endereco} onChange={e=>setFormEdicao({...formEdicao, endereco:e.target.value})} placeholder="Endereço" /><div className="bg-gray-50 p-2 rounded"><label className="text-xs font-bold text-gray-500">Nova Foto:</label><input type="file" className="w-full text-xs" onChange={e=>setNovaFoto(e.target.files[0])} /></div><button className="w-full bg-blue-600 text-white font-bold py-3 rounded mt-2">Salvar</button></form></div></div>)}
      {modalEvento && (<div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center p-4 z-50"><div className="bg-white rounded-xl shadow-2xl w-[95%] md:w-full max-w-md p-6 relative overflow-y-auto max-h-[90vh]"><button onClick={()=>setModalEvento(null)} className="absolute top-4 right-4 text-gray-400">X</button><h2 className="text-2xl font-bold text-gray-800 mb-2">{modalEvento.titulo}</h2>{modalEvento.arquivo_url && <img src={modalEvento.arquivo_url} className="w-full h-40 object-cover rounded mb-4 border"/>}<div className="space-y-2 text-gray-600 mb-6 text-sm"><p>📍 {modalEvento.local}</p><p>💰 {modalEvento.valor > 0 ? `R$ ${modalEvento.valor}` : 'Grátis'}</p><p className="bg-gray-100 p-2 rounded italic">{modalEvento.descricao}</p></div><button onClick={inscrever} className="w-full bg-blue-600 text-white font-bold py-3 rounded-lg">Confirmar Inscrição</button></div></div>)}
      {modalCadastro && (<div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center p-4 z-50"><div className="bg-white rounded-xl shadow-2xl w-[95%] md:w-full max-w-md p-6 relative"><button onClick={()=>setModalCadastro(false)} className="absolute top-4 right-4 text-gray-400">X</button><h2 className="text-xl font-bold text-gray-800 mb-4">Novo Aluno</h2><form onSubmit={cadastrarFilho} className="space-y-3"><input required placeholder="Nome" className="w-full border p-2 rounded text-black" value={novoAluno.nome} onChange={e=>setNovoAluno({...novoAluno, nome:e.target.value})} /><input required type="date" className="w-full border p-2 rounded text-black" value={novoAluno.data_nascimento} onChange={e=>setNovoAluno({...novoAluno, data_nascimento:e.target.value})} /><input required placeholder="Posição" className="w-full border p-2 rounded text-black" value={novoAluno.posicao} onChange={e=>setNovoAluno({...novoAluno, posicao:e.target.value})} /><input required placeholder="Endereço" className="w-full border p-2 rounded text-black" value={novoAluno.endereco} onChange={e=>setNovoAluno({...novoAluno, endereco:e.target.value})} /><input type="file" required className="w-full text-xs" onChange={e=>setArquivoFoto(e.target.files[0])} /><button className="w-full bg-green-600 text-white font-bold py-3 rounded">Salvar</button></form></div></div>)}
    </div>
  )
}